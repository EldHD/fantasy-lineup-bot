...
async def fetch_matches_by_league(code: str, limit: int = 10):
    async with SessionLocal() as session:
        t_res = await session.execute(select(Tournament).where(Tournament.code == code))
...
async def fetch_match_with_teams(match_id: int):
    async with SessionLocal() as session:
        ...
        return {
            ...
            "home": {"id": m.home_team.id if m.home_team else None, "name": m.home_team.name if m.home_team else "TBD"},
            "away": {"id": m.away_team.id if m.away_team else None, "name": m.away_team.name if m.away_team else "TBD"},
        }
...
async def fetch_team_lineup_predictions(match_id: int, team_id: int):
    async with SessionLocal() as session:
        stmt = (
            ...
            .options(selectinload(Prediction.player))
        )
        res = await session.execute(stmt)
